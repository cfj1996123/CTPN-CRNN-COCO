...
@click.option('--abc', type=str, default=string.digits+string.ascii_letters+string.punctuation, help='Alphabet')
...
def main(data_path, abc, seq_proj, backend, snapshot, input_size, base_lr, step_size, max_iter, batch_size, output_dir, test_epoch, test_init, gpu):
    ...
    while True:
        ...
        for sample in iterator:
            ...
            status = "epoch: {}; iter: {}; lr: {}; loss_mean: {}; loss: {}".format(epoch_count, iter_count, lr_scheduler.get_lr(), np.mean(loss_mean), loss.data[0])
        ...
        if output_dir is not None:
            ...
            torch.save(net.state_dict(), os.path.join(output_dir, "crnn_" + backend + "_" + str(epoch_count)))
        epoch_count += 1
